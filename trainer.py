...
class Trainer:
    ...
    def validate_step(self, data_loader, model, visualize=False):
        ...
        for i, batch in tqdm(enumerate(data_loader), total=len(data_loader)):
            pred = model.forward(batch, training=False) # the result of predict by model
            output = self.structure.representer.represent(batch, pred)  # 结果和原图的操作
